...
def checkExists(codisName, yzHostList):
  conn = sqlite3.connect('ClusterManagerTest.db')
...
def rsyncYzRedis(yzHostList):
  for yzHost in yzHostList:
    ret = commands.getoutput("rsync -avzp /home/work/CodisInstall_YzTest/deply_codis_yz/monitor root@%s:/home/work/opbin/" % yzHost)
    print ret
    ret = commands.getoutput("rsync -avzp /home/work/CodisInstall_YzTest/crontab_work root@%s:/var/spool/cron/work" % yzHost)
    print ret
    ret = commands.getoutput("rsync -avzp /home/work/CodisInstall_YzTest/deply_codis_xy/codis_6*  deply_codis_xy/codis_7* root@%s:/home/work/" % yzHost)
    print ret
    ret = commands.getoutput("rsync -avzp /home/work/CodisInstall_YzTest/deply_codis_yz/startCodis.sh root@%s:/home/work/" % yzHost)
    print ret

def createYzConfigini(codisName):
  ret = commands.getoutput("cp /home/work/CodisInstall_YzTest/config.ini.yz.template tmp/config.ini-%s" % codisName)
  print ret
  ret = commands.getoutput('sed -i "s#xxxx#%s#g" /home/work/CodisInstall_YzTest/tmp/config.ini-%s' % (codisName, codisName))
  print ret
  
def rsyncYzDashboard(yzHostList, codisName):
  for yzHost in yzHostList:
    ret = commands.getoutput("rsync -avzp /home/work/CodisInstall_YzTest/deply_codis_yz/dashboard root@%s:/home/work/" % yzHost)
    print ret
    ret = commands.getoutput("rsync /home/work/CodisInstall_YzTest/tmp/config.ini-%s root@%s:/home/work/dashboard/conf/config.ini" % (codisName, yzHost))
    print ret

def rsyncYzProxy(yzHostList, codisName):
  for yzHost in yzHostList:
    ret = commands.getoutput("rsync -avzp /home/work/CodisInstall_YzTest/deply_codis_yz/proxy_9100 root@%s:/home/work/" % yzHost)
    print ret
    ret = commands.getoutput("rsync /home/work/CodisInstall_YzTest/tmp/config.ini-%s root@%s:/home/work/proxy_9100/conf/config.ini" % (codisName, yzHost))
    print ret
    ret = commands.getoutput("rsync -avzp /home/work/CodisInstall_YzTest/deply_codis_yz/proxy_9200 root@%s:/home/work/" % yzHost)
    print ret
    ret = commands.getoutput("rsync /home/work/CodisInstall_YzTest/tmp/config.ini-%s root@%s:/home/work/proxy_9200/conf/config.ini" % (codisName, yzHost))
    print ret
...
def getHostMaxnum(codisName):
    cdscodisName = "cds-" + codisName
    conn = sqlite3.connect('/home/work/CodisInstall_YzTest/ClusterManagerTest.db')
