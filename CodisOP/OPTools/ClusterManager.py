...
def DBSelect(expression):
  ...
  if expression == 'all':
    ...
    cursor = conn.execute('select * from hostinfo where hostname like "cds-' + expression + '%" or ip like "%' + expression +'%" order by hostname ASC')
  for row in cursor:
    ifMatch = True
    if IsIP(expression):
...
def DBSelect2(expression, room):
  ...
  if expression == 'all':
    ...
    cursor = conn.execute('select * from hostinfo where hostname like "cds-' + expression + '%.' + room + '" or ip like "%' + expression +'%" order by hostname ASC')
  for row in cursor:
    ifMatch = True
    if IsIP(expression):
      if expression[0] != '.' and expression.split('.')[0] != row[2].split('.')[0]:
