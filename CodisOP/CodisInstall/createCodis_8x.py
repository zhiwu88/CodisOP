...
def check(codisName, xyHostList, yzHostList):
  ...
  for xyHost in xyHostList:
    ...
    if not ret:
      ...
      exit()
    if not xyHost.startswith("10.0"):
      print "IP:" + xyHost + " is not xyIP"
      exit()

  if not (len(xyHostList)/len(yzHostList)==2 and len(xyHostList)%len(yzHostList)==0):
...
def startYzRedis(yzHostList):
  for yzHost in yzHostList:
    ret = commands.getoutput("ssh %s 'su - work -c \"source /etc/profile && for port in 8000 8001 8002 8003 8004 8005 8006 8007 8008 8009 8010 8011;do cd /home/work/codis_\$port && sh load.sh start;done\"'" % yzHost)
    print ret
...
def createSh(codisName, xyHostList, yzHostList):
  ...
  slaveList = [8000, 8001, 8002, 8003, 8004, 8005, 8006, 8007, 8008, 8009, 8010, 8011]
